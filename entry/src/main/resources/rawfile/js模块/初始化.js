...
window._调试面板 = function(消息) {
  ...
  if (!panel) {
    ...
    panel.id = '_调试面板';
    panel.style.cssText = 'position:fixed;top:60px;right:10px;background:rgba(0,0,0,0.85);color:#0f0;padding:10px;border-radius:6px;font-size:13px;z-index:999999;max-width:90%;max-height:80%;overflow:auto;font-family:monospace;white-space:pre-wrap;';
    document.body.appendChild(panel);
  }
  const line = document.createElement('div');
  line.textContent = '[' + new Date().toLocaleTimeString() + '] ' + 消息;
